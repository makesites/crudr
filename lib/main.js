...
CRUDr.prototype = {
	...
	listen : function(options) {
		...
		if( !_.isEmpty( this.server) )
			this.sockets.init( { server: this.server, db: this.db, config: this.config });
	...
	backends : function() {
		...
		for(var i in backends ){
			...
			var store = this.stores[i] || this.stores;
			var sync = (this.options.sync) ? (this.options.sync[i] || false) : false;

			backend.use({ middleware: this.helpers[name]( store ), sync: sync });
