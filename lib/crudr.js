...
CRUDr.prototype = {
	...
	listen : function(options) {
		...
		if( !_.isEmpty(options.server) ){
			...
			//io.set('log level', 1);
			setupSockets( options.server, options.events );
...
function setupSockets( server, res ){
    ...
    Object.keys(crudr.db).forEach(function(domain) {
        io.of(domain).on('connection', function(socket) {
			...
			socket.on('sync', function(req, callback) {
                ...
                sync.handle(crudr.db[domain], req, function(err, result) {
                    callback(err, result);
            ...
            var events = { 'created': 'create', 'updated': 'update', 'deleted': 'delete' };
            Object.keys(events).forEach(function(event) {
                crudr.db[domain].on(event, function(model) {
