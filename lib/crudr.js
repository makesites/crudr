...
function setupRoutes( server ){
	...
	server[route]( config.path + '/:domain/:id', function(req, res) {
			req.backend = getBackend( req.params.domain );
			// save the method requested
			req.method = routes[route];
			// update the model from the URL, if required
			req.model || (req.model = {});
			req.model.id || (req.model.id = parseInt(req.params.id) );
			
			//console.log( route +', domain: ' + req.params.domain + ' id: ' + req.params.id);
			db[ req.backend ].handle(req, res, function(err) {
				if (err) res.send(err);
			});
			db[ req.backend ].on(events[route], function(model) {
				console.log( model );
			});
...
function setupStatic( server ){
    ...
    server.get( config.static["client.js"], function(req, res){
		...
		fs.readFile( __dirname + '/client.js', function (err, data) {
			...
			data = data.toString().replace(/{{host}}/g, config.host);
			res.write(data);
			res.end();
		});
